...
*	
*	@param		int length			capacity of the buffer in terms of bytes 
*	
...
int resize(ring_t *ring, int length ){
	...
	char* temp = NULL;
	char* ptrPreRing;
	int entries_pre = entries(ring);
	temp = malloc(length);
	if(temp == NULL){
		return -2;
	}
	if(entries_pre>=length){
		ptrPreRing=(char*)(ring->Buffer+ring->Ini);
		if(ring->Full){
			ptrPreRing++;
		...
		ptrPreRing=(char*)(ring->Buffer+ring->Outi);
		for(int i=0;i<entries_pre;i++){
			*(temp+i) = *ptrPreRing;
			ptrPreRing++;
...
*	@param		char * data		pointer to the variable which hold the extraced data if read succesfully
*	
*	@return   	0 if read succesfully;
* 						-1 if input with NULL pointer to ring_t  or NULL pointer to char 

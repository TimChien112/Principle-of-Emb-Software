...
void pattern_check(param* param_input){
	INT8_t cmd_type = check_param_pattern_check(param_input),check_res;
	UINT64_t addr,seed;
	...
	switch (cmd_type){
		case 2:
			...
			uart_num=sprintf(uplink_buffer,"According to the parameters, try to check pseudo random data pattern at: address starting at %lx, range: 0x%x\n\r",addr,range);PRINTF;
			...
			check_res=pattern_check_data((UINT32_t*)addr,range,seed);
			clock_gettime(CLOCK_REALTIME   , &end_time);		
			delta_t(&end_time, &start_time, &delta_time);
			if(check_res==1){
				uart_num=sprintf(uplink_buffer,"pattern is identical with input seed: %lx\n\r",seed);PRINTF;
			}
			else{
				uart_num=sprintf(uplink_buffer,"pattern is different with input seed: %lx\n\r",seed);PRINTF;
			}
			uart_num=sprintf(uplink_buffer,"checking pseudo random data pattern finished at %ld sec, %ld microsec (%ld nanosec)\n\r", delta_time.tv_sec, (delta_time.tv_nsec / NSEC_PER_MICROSEC), (delta_time.tv_nsec));PRINTF;
			break;
		case 1:
			...
			uart_num=sprintf(uplink_buffer,"According to the parameters, try to check pseudo random data pattern at : Block%d, offset starting at 0x%x, range: 0x%x\n\r",index,offset,range);PRINTF;
			if(check_blk(index,offset) && check_blk( index,offset+range-1) ){
				clock_gettime(CLOCK_REALTIME   , &start_time);
				check_res=pattern_check_data(mem[index].mem_ptr+offset,range,seed);
				clock_gettime(CLOCK_REALTIME   , &end_time);		
				delta_t(&end_time, &start_time, &delta_time);
				if(check_res==1){
					uart_num=sprintf(uplink_buffer,"pattern is identical with input seed: %lx\n\r",seed);PRINTF;
